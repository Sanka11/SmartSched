...
const Sidebar = ({
  ...
  const roles = ["admin", "lecturer", "student", "course manager", "assignment manager", "user manager", "user"];
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed lg:static z-30 w-64 bg-gradient-to-b from-indigo-700 to-blue-800 text-white transition-all duration-300 ease-in-out 
      ...
      >
        ...
        <nav className="p-4 flex flex-col h-[calc(100%-64px)]">
          ...
          <div className="flex-1">
            <div className="mb-8">
              <h2 className={`text-xs uppercase font-semibold mb-4 ${!sidebarOpen && "text-center"}`}>
              ...
              <ul className="space-y-2">
                <li>
                  <a 
                    href="#"
                    className="flex items-center p-2 rounded-lg bg-blue-900 text-white"
                    aria-current="page"
                  >
                ...
                {roles.map((role) => (
                  <li key={role}>
                    <button
                      ...
                      className={`flex items-center w-full p-2 rounded-lg transition-colors ${activeTab === role ? 'bg-blue-600' : 'hover:bg-blue-700'}`}
                      aria-label={`View ${role} users`}
                    >
                      {roleIcons[role]}
                      {sidebarOpen && <span>{role.charAt(0).toUpperCase() + role.slice(1)}</span>}
                    </button>
                  </li>
                ))}
              </ul>
