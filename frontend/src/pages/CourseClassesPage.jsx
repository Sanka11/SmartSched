...
const CourseClassesPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [coursesRes, instructorsRes, assignmentsRes, locationsRes] =
          await Promise.all([
        ...
        setLocations(locationsRes.data.map((loc) => loc.hallName));
      } catch (err) {
  ...
  const fetchExistingGroupDetails = async (course) => {
    ...
    try {
      const response = await api.get(
        `/api/allClassAssignments?courseId=${course.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const saveGroupDetails = async (groupId, moduleName) => {
    ...
    const course = courses.find((c) =>
      c.groups?.some((g) => g.groupId === groupId)
    );
    ...
    if (!course || !group || !module || assignedInstructors.length === 0) {
      toast.error(
        "Invalid data. Please check the course, group, module, or instructor assignments."
      );
      return;
    }
    ...
    for (const instructor of assignedInstructors) {
      if (
        ...
        toast.error(
          `Instructor ${instructor.firstName} ${instructor.lastName} is already booked during the selected time period.`
        );
        return;
      }
    }
    ...
    try {
      ...
      const savePromises = assignedInstructors.map(async (instructor) => {
        ...
        if (!response.ok)
          throw new Error(
            `Failed to save data for instructor ${instructor.id}`
          );
      ...
      toast.success(
        `Details ${
          details.id ? "updated" : "saved"
        } for Group ID: ${groupId}, Module: ${moduleName}`
      );
  ...
  const deleteGroupDetails = async (groupId, moduleName) => {
    ...
    try {
      ...
      toast.success(
        `Deleted assignment for Group ID: ${groupId}, Module: ${moduleName}`
      );
                              ...
                              <div className="mt-3 flex justify-between items-center">
                                ...
                                {assignedInstructors.length === 0 && (
                                  <span className="text-sm text-red-500">
                                    No instructors assigned to this module
                                  </span>
                                )}
                              </div>
                            </div>
