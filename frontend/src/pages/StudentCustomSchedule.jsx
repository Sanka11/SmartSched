...
const StudentCustomSchedule = () => {
  ...
  const handlePromptChoice = async (choice) => {
    setShowPrompt(false);
    localStorage.setItem("customPromptShownFor", timetableTimestamp);
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      doc.text("Custom Schedule", pageWidth / 2, 30, { align: "center" });
      ...
      hours.forEach((hour, rowIndex) => {
        ...
        days.forEach((day, colIndex) => {
          ...
          const personal = personalSlots[getSlotId(day, hour)];
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-indigo-50">
                    <th className="border-b border-gray-200 p-3 text-left text-sm font-medium text-gray-700 sticky left-0 bg-indigo-50 z-10 min-w-[80px]">
                      Time
                    </th>
                    {days.map((day) => (
                      <th
...
export default StudentCustomSchedule;
